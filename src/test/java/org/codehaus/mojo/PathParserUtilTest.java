...
public class PathParserUtilTest extends TestCase
{
     ...
     */
    private ExistingResources getExistingResourcesAndWildcard() throws IOException
    {
        File curDir = new File(".");
        String list[] = curDir.list();
        if (list == null || list.length == 0)
        {
            ...
            {
                // return the path looking like: ../parentDir/c* for curDir
                return new ExistingResources("../" + curDir.getParentFile().getName(), toWildCard(curDir.getName()), isCaseSensitive);
            }
            else
            {
                return null;
     ...
     */
    private void assureValidFiles(String path, Set expectedFiles) throws MojoExecutionException
    {
        Set set = PathParserUtil.toFiles(path, isCaseSensitive);
        assertEquals(expectedFiles, set);
    }

    private void assureInvalidFiles(String path, Set expectedFiles) throws MojoExecutionException
    {
        Set set = PathParserUtil.toFiles(path, isCaseSensitive);
     ...
     * and given wildcard.
     */
    private void assureValid(String string, String path, String wildcard) throws MojoExecutionException
    {
